...
class Handler implements HandlerOptions {
	...
	public setVisible1 = (visible?: boolean) => {
		const activeObject = this.canvas.getActiveObject() as FabricTicker;
		if (!activeObject) {
			return;
		}
		if (activeObject.superType === 'element') {
	...
	public getScreenshotOfElement = () => {
		html2canvas(document.querySelector(".rde-editor-canvas"))
    .then((canvas) => {
