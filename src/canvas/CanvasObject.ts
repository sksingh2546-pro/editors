...
const CanvasObject: CanvasObjectSchema = {
	...
	ticker: {
		create: ({ code, ...option }: { code: Code }) => new Ticker(code, option),
	},
	imageScroll: {
		create: ({ code, ...option }: { code: Code }) => new ImageScroll(code, option),
	},
	rating: {
		create: ({ src, ...option }: { src: string }) => new Rating(src, option),
	},
	iframe: {
		create: ({ src, ...option }: { src: string }) => new Iframe(src, option),
	},
	weather: {
		create: ({ src, ...option }: { src: string }) => new Weather(src, option),
	},
	playlist: {
		create: ({ code, ...option }: { code: Code }) => new Playlist(code, option),
	},
	clock: {
		create: ({ src, ...option }: { src: string }) => new Clock(src, option),
	},
	video: {
