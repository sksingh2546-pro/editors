...
class ImageMapEditor extends Component {
	...
	async componentDidMount() {
        ...
        debugger
		this.canvasRef.handler.zoomHandler.zoomDefault()
		if (sessionStorage.getItem("appMedia")) {
			const resp = await axios.get(sessionStorage.getItem("appMedia").split(",")[0].replace("pub", "drft"));

			const { objects, animations, styles, dataSources } = resp.data;
			this.setState({
				animations,
				styles,
				dataSources,
			...
			if (objects) {
				...
				data.map((value, i) => {
					...
					if (value.name === "ratings") {
						value.width = "1920";
						value.height = "1080";
						value.scaleX = 1;
						value.scaleY = 1;
						value.type = "rating"
						sessionStorage.setItem("displayUrl", value.src.replace("cms.",""))
					}
					else if (value.name === "clock") {
						value.type = "clock"
						sessionStorage.setItem("clockUrl", value.src.replace("cms.",""))
					}
					else if (value.name === "weather") {
						value.type = "weather"
						sessionStorage.setItem("weatherUrl", value.src.replace("cms.",""))
					}
					newData.push(value)
				})
	...
	handlers = {
		...
		onSave: async () => {
			...
			const datas = {
				appId: sessionStorage.getItem("selectedAppId"),
				appMedia: sessionStorage.getItem("appMedia"),
				appName: sessionStorage.getItem("selectedAppName"),
				width: sessionStorage.getItem("width"),
				height: sessionStorage.getItem("height"),
				imgBase64:this.canvasRef.handler.saveCanvasImage(),
				jsonCanvas: JSON.stringify(exportDatas),
				orientation: "landscape"
			}
			const header = {
				'Authorization': `Bearer ${sessionStorage.getItem("jwtToken")}`
			}
			const data = await axios.post("https://summitsignage.com/api/custom-design", datas, { headers: header })
	...
	render() {
		...
		const action = (
			<React.Fragment>
				...
				<Button style={{ background: "rgb(102, 72, 249)", color: "#fff", margin: 2 }}
					onClick={() => { 
				...
				<Button style={{ background: "rgb(102, 72, 249)", color: "#fff", margin: 2 }} disabled={!editing}
					onClick={onSave}
				><Icon name="save" />
					&nbsp;&nbsp;&nbsp;
					Save
