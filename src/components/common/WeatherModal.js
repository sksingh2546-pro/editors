...
class UrlModal extends Component {
	handlers = {
		onOk: () => {
			...
			onChange({ tempUrl: sessionStorage.getItem("weatherUrl") });
			this.setState({
				visible: false,
				url: { tempUrl: sessionStorage.getItem("weatherUrl") },
			});
		},
		onCancel: () => {
				...
				<Modal
					...
					<div
						dangerouslySetInnerHTML={{
							__html: `<iframe 
				  title='WebView'
				  src=${sessionStorage.getItem("weatherUrl") ? sessionStorage.getItem("weatherUrl") + "&set=true" : "https://summitsignage.com/pub/plugins/weather.html?set=true&wthrsetting=en~wl653~image~3~fahrenheit~one_a~0~0~0~0~landscape&zoom=1.2"} 
				  height="500px"
				  width="100%" 
				  scrolling={true} 
				  frameBorder='0' 
