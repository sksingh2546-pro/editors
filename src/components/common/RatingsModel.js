...
class UrlModal extends Component {
	handlers = {
		onOk: () => {
			...
			onChange({ tempUrl: sessionStorage.getItem("displayUrl") });
			this.setState({
				visible: false,
				url: { tempUrl: sessionStorage.getItem("displayUrl") },
			});
		},
		onCancel: () => {
	...
	render() {
		...
		window.addEventListener("message", (event) => {
			...
			console.log("djkjjkdsjksadjk"+event.data);
				...
				<Modal
					...
					<div
						dangerouslySetInnerHTML={{
							__html: `<iframe 
				  title='WebView'
				  src=${sessionStorage.getItem("displayUrl") ? sessionStorage.getItem("displayUrl").replace(/ /g, '%20') + "&set=true" : "https://summitsignage.com/pub/plugins/ratings.html?set=true&fbs_url=https://apps.summitsignage.com&username="+userInfo.username+"&bg_color=ee3e3e&fontfam=Verdana&fontCol=FFFFFF&display_list=".replace(/ /g, '%20')} 
				  height="500px"
				  width="100%" 
				  scrolling={true} 
				  frameBorder='0' 
